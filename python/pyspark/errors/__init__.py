...
# Import the upstream errors module. Since we're in a connect-only client,
# we need access to the official Spark error definitions for testing.
try:
    # Try to import from the upstream Apache Spark pyspark (if installed)
    from sys import modules

    # Temporarily remove our pyspark from the path to get the upstream one
    our_pyspark = modules.get('pyspark')
    our_init = modules.get('pyspark.__init__')

    # Dynamically import from upstream using exec to avoid circular imports
    _upstream_errors = {}
    import importlib.util
    spark_errors_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)),
        "..",  # Go up from our pyspark-client-fast
        "spark",
        "python",
        "pyspark",
        "errors"
    ...
    # Actually, we should just try importing directly from the spark checkout
    # which is on sys.path already
    import sys
    # Save our pyspark entry
    sys_modules_pyspark = sys.modules.get('pyspark')
    sys_modules_pyspark_errors = sys.modules.get('pyspark.errors')

    # Try to import from Apache Spark's pyspark
